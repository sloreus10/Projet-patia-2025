...
(:predicates 
    ...
    (aVoisinDroit ?x - sol ?y - sol)            ;aVoisinDroit : Spécifie que le sol ?y est à droite du sol ?x (indiréctement le sol ?x est à gauche du sol ?y)
    (aVoisinHaut ?x - sol ?y - sol)             ;aVoisinHaut : Spécifie que le sol ?y est en haut du sol ?x (indiréctement le sol ?x est en bas du sol ?y)
